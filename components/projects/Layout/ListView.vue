...
interface HeaderColumn {
    name: string
    id: string // Key to access dataValue
    style: string
}
...
/**
 * Resolves a nested value from an object using a dot-separated string.
 */
const resolveNestedValue = (data: any, key: string): any => {
                ...
                >
                    ...
                    >
                        <template v-if="header.id.includes('.')">
                            {{ resolveNestedValue(dataValue, header.id) }}
                        </template>
                        <template v-else>
                            {{ dataValue[header.id] }}
                        </template>
                    ...
                    <td v-if="actions" class="p-2 flex gap-2 justify-center">
                        <button v-if="actions.edit" @click="doEdit(dataValue)">
                            <Icon
                                ...
                                color="white"
                                class="bg-green-400 rounded h-8 w-8 p-1"
                            />
                        </button>
                        ...
                        <button v-if="actions.detail" @click="doDetail(dataValue)">
                            <Icon
                                name="material-symbols:visibility-rounded"
                                color="white"
                                class="bg-green-500 rounded h-8 w-8 p-1"
                            />
                        </button>
                    </td>
                </tr>
