...
<template>
    <div v-if="datas && datas.length > 0" class="pb-2 text-gray-500 text-[12px] overflow-y-auto p-2">
        <table class="table-fixed w-full border-collapse overflow-y-auto">
            <thead>
                <tr class="border">
                    <th v-for="header in headerColumns" :key="header.name+'headerRow'" :colspan="header.colSpan ? header.colSpan : 1" class="p-2 border" :class="header.style ?? ''">
                        {{ header.name }}
                    </th>
                ...
                <tr v-if="subHeaders" class="border">
                    <th v-for="subHeader in subHeaders" :key="subHeader.name+'subHeaderRow'" class="p-2 border">
                        {{ subHeader.name }}
                    </th>
                </tr>
            </thead>
            <tbody>
                <tr v-for="dataValue, index in datas" :key="index" class="border text-center" :class="{ 'active': isActiveRow(index) }" @click="setActiveRow(index)">
                    <td v-for="header in headerColumns" :key="header.name+'headerRow'" :colspan="header.colSpan ? header.colSpan : 1" class="p-2 border" :class="header.style ?? ''">
                    ...
                    <td v-if="actions" class=" p-2 flex gap-2 justify-center border">
