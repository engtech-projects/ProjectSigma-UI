...
const { myDeniedRequests } = storeToRefs(paymentRequestStore)
...
const changePaginate = (newParams) => {
    myDeniedRequests.value.params.page = newParams.page ?? ""
}
const headers = [
...
<template>
    <LayoutBoards class="w-full" :loading="myDeniedRequests.isLoading">
        ...
        >
            <div class="w-1/3 flex">
                <LayoutFormPsTextInput v-model="myDeniedRequests.params.key" title="PRF Number" class="w-full" />
            </div>
            <div class="pb-2 text-gray-500 text-[12px] overflow-y-auto p-2">
                <LayoutPsTable
                    :header-columns="headers"
                    :actions="actions"
                    :datas="myDeniedRequests.list ?? []"
                    @show-table="showInformation"
                />
                <div class="flex justify-center mx-auto">
                    <PsCustomPagination :links="myDeniedRequests.pagination" @change-params="changePaginate" />
                </div>
            </div>
        </LayoutAcessContainer>
    </LayoutBoards>
