...
const { cashClearingVoucher } = storeToRefs(voucherStore)

const requestData = ref(null)
const showInformationModal = ref(false)
await voucherStore.getClearingVouchers()
const showInformation = (data) => {
    requestData.value = data
    showInformationModal.value = true
}
const changePaginate = (newParams) => {
    cashClearingVoucher.value.params.page = newParams.page ?? ""
}
const headers = [
...
<template>
    <LayoutBoards class="w-full" :loading="cashClearingVoucher.isLoading">
        ...
        >
            <div class="w-1/3 flex">
                <LayoutFormPsTextInput v-model="cashClearingVoucher.params.key" title="CV Number/Journal Voucher Number" class="w-full" />
            </div>
            <div class="pb-2 text-gray-500 text-[12px] overflow-y-auto p-2">
                <LayoutPsTable
                    :header-columns="headers"
                    :actions="actions"
                    :datas="cashClearingVoucher.list ?? []"
                    @show-table="showInformation"
                />
                <div class="flex justify-center mx-auto">
                    <CustomPagination :links="cashClearingVoucher.pagination" @change-params="changePaginate" />
                </div>
            </div>
        </LayoutAcessContainer>
    </LayoutBoards>
