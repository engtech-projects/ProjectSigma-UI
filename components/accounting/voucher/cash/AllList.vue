...
const { allCashVouchers } = storeToRefs(voucherStore)

const requestData = ref(null)
const showInformationModal = ref(false)
await voucherStore.getAllCashVouchers()
const showInformation = (data) => {
    requestData.value = data
    showInformationModal.value = true
}
const changePaginate = (newParams) => {
    allCashVouchers.value.params.page = newParams.page ?? ""
}
const headers = [
...
<template>
    <LayoutBoards class="w-full" :loading="allCashVouchers.isLoading">
        ...
        >
            <div class="w-1/3 flex">
                <LayoutFormPsTextInput v-model="allCashVouchers.params.key" title="CV Number/Journal Voucher Number" class="w-full" />
            </div>
            <div class="pb-2 text-gray-500 text-[12px] overflow-y-auto p-2">
                <LayoutPsTable
                    :header-columns="headers"
                    :actions="actions"
                    :datas="allCashVouchers.list ?? []"
                    @show-table="showInformation"
                />
                <div class="flex justify-center mx-auto">
                    <PsCustomPagination :links="allCashVouchers.pagination" @change-params="changePaginate" />
                </div>
            </div>
        </LayoutAcessContainer>
    </LayoutBoards>
