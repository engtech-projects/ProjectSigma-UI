<template>
  <form action="" v-for="(emprecord, index) in emprecords" :key="index">
    <div class="flex items-center justify-end">
        ...
        <button class="add-button" @click="addEmprecords" v-if="index === emprecords.length - 1">
          <span class="text-green-500 hover:text-green-700">Add References Record</span>
        </button>
      </div>
        ...
        <tbody>
          <tr >
              ...
              <label for="small-input" class="block mb-2 text-[11px] font-medium text-gray-900 dark:text-white">POSITION</label>
                <div class="flex-wrap">
                    <input type="text" v-model="emprecord.supcont" id="inclusive_date"  class="block w-36 p-1 text-gray-900 border border-gray-300 rounded-md bg-gray-50 sm:text-xs focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                </div>
              </td>
              <td class="border border-slate-300 p-1">
