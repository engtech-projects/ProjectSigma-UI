...
stockTransferStore.fetchTransfers()
...
const filterWarehouse = async () => {
    ...
    await stockTransferStore.fetchTransferByWarehouseId(wareHouseId.value)
    if (stockTransferStore.errorMessage !== "") {
        snackbar.add({
            type: "error",
            text: stockTransferStore.errorMessage
        })
    }
    stockTransferStore.errorMessage = ""
}
