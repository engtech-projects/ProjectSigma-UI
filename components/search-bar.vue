<template>
  <div>
    ...
    <ul v-if="showSuggestions" class="mt-1 p-px rounded-lg bg-white dark:bg-gray-800">
      <li v-if="filteredEmployees.length === 0" class="flex justify-center p-1 italic">No Records Found</li>
      <li v-for="employee in filteredEmployees" :key="employee.id" @click="selectEmployee(employee)" class="hover:bg-teal-200 rounded-md cursor-pointer p-1">
        {{ employee.name }}
      </li>
    </ul>
  </div>
