...
const { listSchedulesProject, updateScheduleRequestProject } = storeToRefs(scheduleStore)
watch(listSchedulesProject.value.params, () => {
    debouncedGetData()
}, { deep: true })
...
const calendarOptions = ref({
    ...
    events: function (event: any, successCallback: any) {
        listSchedulesProject.value.params.start_date = event.start.toISOString().slice(0, 10)
        listSchedulesProject.value.params.end_date = event.end.toISOString().slice(0, 10)
        successCallback(listSchedulesProject.value.data)
    }
})
const setEdit = (id: any) => {
    const schedule = listSchedulesProject.value.data.find((item: any) => parseInt(item?.id) === parseInt(id))
    updateScheduleRequestProject.value.body = schedule
    updateScheduleRequestProject.value.show = true
}
...
<template>
    <LayoutBoards
        ...
        :loading="listSchedulesProject.isLoading"
    >
        <template #default>
            <div class="flex flex-col gap-2">
                <HrmsCommonProjectSelector v-model="listSchedulesProject.params.project_id" />
