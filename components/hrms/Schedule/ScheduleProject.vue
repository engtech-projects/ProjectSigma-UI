...
export default {
    ...
    methods: {
        async fetchProjects () {
            await useFetch(
                "/api/project-monitoring/project",
                {
                    ...
                    onResponse: ({ response }) => {
                        ...
                        if (response.status !== 200) {
                            ...
                            this.projects = response._data.data.data
                        }
                    },
                }
            )
        ...
        resetEvents () {
            this.isEdit = false
            this.daysOfWeek = [false, false, false, false, false, false, false]
            const currentProject = this.newEvent.project_id
            this.newEvent = {
                id: 1,
            ...
            this.newEvent.project_id = currentProject
        },
        formatTime (time) {
        ...
        loadEvents () {
            this.removeEvents()
            this.events.forEach((ev) => {
                if (ev.project_id === this.newEvent.project_id) {
