                    ...
                    <template #eventContent="{ event }">
                        <div class="event-container flex flex-col justify-between w-full items-center p-1 bg-blue-600 text-[10px] overflow-hidden text-white rounded-md">
                            ...
                            <div class="flex gap-[5px]">
                                <button
                                    type="button"
                                    class="hover:bg-slate-700 rounded"
                                    aria-label="Edit Schedule"
                                    title="Edit"
                                    @click.stop="setEdit(event.id)"
                                    @keydown.enter.stop.prevent="setEdit(event.id)"
                                >
                                    <svg
                                        class="text-slate-200 h-[10px] w-[10px]"
                                        fill="none"
                                        stroke="currentColor"
                                        viewBox="0 0 24 24"
                                ...
                                <button
                                    ...
                                    title="Delete"
                                    @click.stop="deleteSchedule(event.id)"
                                    @keydown.enter.stop.prevent="deleteSchedule(event.id)"
                                >
                                    <svg
                                        class="text-slate-200 h-[10px] w-[10px]"
                                        fill="none"
