...
const { listSchedulesEmployee, updateScheduleRequestEmployee } = storeToRefs(scheduleStore)
watch(listSchedulesEmployee.value.params, () => {
    debouncedGetData()
}, { deep: true })
...
const calendarOptions = ref({
    ...
    events: function (event: any, successCallback: any) {
        listSchedulesEmployee.value.params.start_date = event.start.toISOString().slice(0, 10)
        listSchedulesEmployee.value.params.end_date = event.end.toISOString().slice(0, 10)
        successCallback(listSchedulesEmployee.value.data)
    }
})
const setEdit = (id: any) => {
    const schedule = listSchedulesEmployee.value.data.find((item: any) => parseInt(item?.id) === parseInt(id))
    updateScheduleRequestEmployee.value.body = schedule
    updateScheduleRequestEmployee.value.show = true
}
...
<template>
    <LayoutBoards
        ...
        :loading="listSchedulesEmployee.isLoading"
    >
        <template #default>
            <div class="flex flex-col gap-2">
                <HrmsCommonSearchEmployeeSelector v-model="listSchedulesEmployee.params.employee_id" />
