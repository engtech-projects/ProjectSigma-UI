<script setup>

</script>
<template>
    <div class="flex flex-col">
        <div class="header flex flex-col gap-1 mb-16">
            <span class="text-xl font-bold text-blue-500">
                EMPLOYER NO: 80-0191406-1-000
            </span>
            <span class="text-2xl font-bold text-blue-500">
                EVENPAR CONSTRUCTION AND DEVELOPMENT CORPORATION
            </span>
            <span class="text-xl text-blue-500">
                P-1 POBLACION 8 BUENAVISTA AGUSAN DEL NORTE
            </span>
        </div>
        <div class="title flex flex-col justify-center gap-1 mb-12">
            <span class="text-xl text-black text-center">
                MONTH OF <span class=" font-bold underline">APRIL 2024</span>
            </span>
        </div>
        <span class="text-lg">
            OFFICE - PROJECT MANAGEMENT SECTION
        </span>
        <table class="printTable border border-gray-500 border-2">
            <thead class="text-blue-600 text-md bg-blue-300">
                <tr class="py-4">
                    <th rowspan="3" class="py-4 border-gray-500 border-2">
                        NO.
                    </th>
                    <th rowspan="1" class="border border-gray-500 border-2">
                        NAME OF EMPLOYEE
                    </th>
                    <th rowspan="3" class="border border-gray-500 border-2">
                        NO. OF EMPLOYEES
                    </th>
                    <th colspan="3" class="border border-gray-500 border-2">
                        PHIC AMOUNT
                    </th>
                </tr>
                <tr>
                    <th rowspan="2" class="border border-gray-500 border-2">
                        SURNAME
                    </th>
                    <th rowspan="2" class="py-4 border-gray-500 border-2">
                        PS
                    </th>
                    <th rowspan="2" class="border border-gray-500 border-2">
                        ES
                    </th>
                    <th rowspan="2" class="border border-gray-500 border-2">
                        TOTAL CONTRIBUTION
                    </th>
                </tr>
            </thead>
            <tbody class="text-sm">
                <tr class="h-2">
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        1
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm">
                        OFFICE - ADMIN
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        6
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        10,100.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        4,200.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        14,300.00
                    </td>
                </tr>
                <tr class="h-2">
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        1
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm">
                        OFFICE - ADMIN
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        6
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        10,100.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        4,200.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        14,300.00
                    </td>
                </tr>
                <tr class="h-2">
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        1
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm">
                        OFFICE - ADMIN
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        6
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        10,100.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        4,200.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        14,300.00
                    </td>
                </tr>
                <tr class="h-2">
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        1
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm">
                        OFFICE - ADMIN
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        6
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        10,100.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        4,200.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        14,300.00
                    </td>
                </tr>
                <tr class="h-2">
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        1
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm">
                        OFFICE - ADMIN
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        6
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        10,100.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        4,200.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        14,300.00
                    </td>
                </tr>
                <tr class="h-2">
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        1
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm">
                        OFFICE - ADMIN
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        6
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        10,100.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        4,200.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        14,300.00
                    </td>
                </tr>
                <tr class="h-2">
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        1
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm">
                        OFFICE - ADMIN
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        6
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        10,100.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        4,200.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        14,300.00
                    </td>
                </tr>
                <tr class="h-2">
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        1
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm">
                        OFFICE - ADMIN
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        6
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        10,100.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        4,200.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        14,300.00
                    </td>
                </tr>
                <tr class="h-2">
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        1
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm">
                        OFFICE - ADMIN
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        6
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        10,100.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        4,200.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        14,300.00
                    </td>
                </tr>
                <tr class="h-2">
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        1
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm">
                        OFFICE - ADMIN
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        6
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        10,100.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        4,200.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        14,300.00
                    </td>
                </tr>
                <tr class="h-2">
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        1
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm">
                        OFFICE - ADMIN
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        6
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        10,100.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        4,200.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        14,300.00
                    </td>
                </tr>
                <tr class="h-2">
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        1
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm">
                        OFFICE - ADMIN
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        6
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        10,100.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        4,200.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        14,300.00
                    </td>
                </tr>
                <tr class="h-2">
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        1
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm">
                        OFFICE - ADMIN
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        6
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        10,100.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        4,200.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        14,300.00
                    </td>
                </tr>
                <tr class="h-2">
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        1
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm">
                        OFFICE - ADMIN
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        6
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        10,100.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        4,200.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        14,300.00
                    </td>
                </tr>
                <tr class="h-2">
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        1
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm">
                        OFFICE - ADMIN
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        6
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        10,100.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        4,200.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        14,300.00
                    </td>
                </tr>
                <tr class="h-2">
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        1
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm">
                        OFFICE - ADMIN
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        6
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        10,100.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        4,200.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        14,300.00
                    </td>
                </tr>
                <tr class="h-2">
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        1
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm">
                        OFFICE - ADMIN
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        6
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        10,100.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        4,200.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        14,300.00
                    </td>
                </tr>
                <tr class="h-2">
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        1
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm">
                        OFFICE - ADMIN
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        6
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        10,100.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        4,200.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        14,300.00
                    </td>
                </tr>
                <tr class="h-2">
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        1
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm">
                        OFFICE - ADMIN
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        6
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        10,100.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        4,200.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        14,300.00
                    </td>
                </tr>
                <tr class="h-2">
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        1
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm">
                        OFFICE - ADMIN
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-center">
                        6
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        10,100.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        4,200.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 text-sm text-right">
                        14,300.00
                    </td>
                </tr>
                <tr>
                    <td colspan="3" class="border border-gray-500 border-2 h-8 px-2 font-bold text-sm text-left">
                        SUB-TOTAL
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 font-bold text-sm text-right">
                        42,200.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 font-bold text-sm text-right">
                        140,300.00
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 font-bold text-sm text-right">
                        100,000.00
                    </td>
                </tr>
                <tr>
                    <td colspan="5" class="border border-gray-500 border-2 h-8 px-2 font-bold text-sm text-left">
                        TOTAL AMOUNT DUE
                    </td>
                    <td class="border border-gray-500 border-2 h-8 px-2 font-bold text-sm text-right">
                        100,000.00
                    </td>
                </tr>
            </tbody>
        </table>
    </div>
</template>
