...
const { paymentData } = storeToRefs(loanStore)
...
const submitPayment = async () => {
    try {
        await loanStore.submitPayment(props.data.id)
        snackbar.add({
            type: "success",
    ...
    <div v-if="!showPaymentForm" class="flex flex-col gap-2">
        ...
        <div class="flex flex-col mt-6">
            <div class="flex items-center justify-between border-b pb-2">
                ...
                <button v-if="!data.is_fully_paid && useCheckAccessibility([AccessibilityTypes.HRMS_LOANSANDADVANCES_LOANS_MAKEPAYMENTS])" class="bg-green-500 rounded-md px-4 py-1 text-white hover:bg-green-600 active:bg-green-700" @click="showMakePayment()">
