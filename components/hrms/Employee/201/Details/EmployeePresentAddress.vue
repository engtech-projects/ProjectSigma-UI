...
<template>
    <div class="flex justify-start mt-3 gap-3">
        ...
        <div v-if="editable" class="mt-2">
            <button v-if="employee.presentAddressParams.id" class=" bg-yellow-400 text-white w-8 h-8" @click="updateEmployeeAddress(employee.presentAddressParams.id)">
                <Icon name="ion:pencil" color="white" class="rounded h-6 w-6 p-1" />
            </button>
            <button v-else class=" bg-green-600 text-white w-8 h-8" @click="saveEmployeeAddress('present')">
