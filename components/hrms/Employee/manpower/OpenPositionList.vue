...
const { openPositions } = storeToRefs(manpowers)
const debouncedGetData = useDebouncedFn(() => {
    manpowers.getOpenPositions()
}, 500)
onMounted(() => {
    if (!openPositions.value.isLoaded) {
        debouncedGetData()
    }
})
watch(
    () => ({ ...openPositions.value.params }),
    (newParams, oldParams) => {
        if (newParams.page === oldParams.page) {
            openPositions.value.params.page = 1
        }
        debouncedGetData()
    },
    { deep: true }
...
<template>
    <LayoutLoadingContainer class="w-full" :loading="openPositions.isLoading">
            ...
            <div class="pb-2 text-gray-500 text-[12px] overflow-y-auto p-2">
                <table class="table-auto w-full border-collapse">
                    ...
                    <tbody>
                        ...
                        <template v-else>
                            <tr v-for="dataValue, index in openPositions.list" :key="index" class="border text-center">
                                <td
                                    class="p-2"
                                >
        ...
        <div class="flex justify-center mx-auto p-2">
            <PsCustomPagination :links="openPositions.pagination" @change-params="changePaginate" />
        </div>
